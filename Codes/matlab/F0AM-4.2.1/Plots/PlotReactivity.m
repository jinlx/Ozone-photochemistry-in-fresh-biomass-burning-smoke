...
% Lixu, fix the error
ParsePairs(varargin(1:end-3),varInfo);
...
%disp(sumflag);
%disp(allflag);
%disp(length(Rct2plot)); % VOCs we set up
%exit()
...
%disp(size(rSp));
%disp(size(rSpnames));
%disp(rSpnames); % all rxns having OH either on the left or the right.
%exit() % test
...
if plotme
    ...
    % if cols==0 && length(cmapname)==0; load 'fillcolors.mat'; cols=colormap(fillcolors); disp('ZZZZZZZZZZZZ');exit();end 
...
% Show the initial OHR
disp('This is the init OHR for NMVOC! ');
disp(sum(R2plot(1,:)));
%disp(saveout);

%%%%%OUTPUT%%%%%
% Lixu, 2023/02/23, modified.
if saveout
    React.Time = Time;
    React.OHR  = R2plot;
    React.lnames = lnames;
    ...
    % save out the data, Lixu, 2023/02/23
    switch Flight_ID
        case {'FN06', 'FN19'}
            filename = ['/glade/u/home/lixujin/matlab/F0AM-4.2.1/Setups/Examples/Lixu/FIREX-AQ/output_data/Reactivity', names, MECHANISM, '_', FixNOx_postfix, Flight_ID];
            save(filename, '-struct', 'React')
            disp('save out required Reactivity data')
        case {'RF03', 'RF07', 'RF09'}
            filename = ['/glade/u/home/lixujin/matlab/F0AM-4.2.1/Setups/Examples/Lixu/WE-CAN/output_data/Reactivity', names, MECHANISM, '_', FixNOx_postfix, Flight_ID];
            save(filename, '-struct', 'React')
            disp('save out required Reactivity data')
        case {'P3B'}
            filename = ['/glade/u/home/lixujin/matlab/F0AM-4.2.1/Setups/Examples/Lixu/P-3B/output_data/Reactivity', names, MECHANISM, '_', FixNOx_postfix, Flight_ID];
            save(filename, '-struct', 'React')
            disp('save out required Reactivity data')
        otherwise
    end    
