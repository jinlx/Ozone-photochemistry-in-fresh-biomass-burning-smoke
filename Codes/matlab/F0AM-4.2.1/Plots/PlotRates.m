...
% Lixu, fix the error
ParsePairs(varargin(1:end-4),varInfo);
...
%%%%%OUTPUT%%%%%
% Lixu, 2022/11/15, modified.
if saveout
    % Fix the error in empty Pnames and Lnames.
    % Find the index of the empty data in Lnames
    index_loss_bad = find(cellfun('isempty', Lnames), 1);
    % Display the index of the empty data (or -1 if not found)
    if ~isempty(index_loss_bad)
        fprintf('The empty data is at loss index %d\n', index_loss_bad);
        % Remove the empty cells and corresponding columns in other arrays
        Lnames(index_loss_bad)           = [];
        L2plot(:,index_loss_bad)         = [];
        iRx_Loss(index_loss_bad)         = [];
        iRx_Loss_grouped(index_loss_bad) = [];
    else
        fprintf('The empty data was not found in the cell array\n');
    end

    % Find the index of the empty data in Lnames
    index_prod_bad = find(cellfun('isempty', Pnames), 1);
    % Display the index of the empty data (or -1 if not found)
    if ~isempty(index_prod_bad)
        fprintf('The empty data is at prod index %d\n', index_prod_bad);
        % Remove the empty cells and corresponding columns in other arrays
        Pnames(index_prod_bad)           = [];
        P2plot(:,index_prod_bad)         = [];
        iRx_Prod(index_prod_bad)         = [];
        iRx_Prod_grouped(index_prod_bad) = [];
    else
        fprintf('The empty data was not found in the cell array\n');
    end
    ...
    names          = varargin{end-3};    
    Flight_ID      = varargin{end-2};
    MECHANISM      = varargin{end-1};
    FixNOx_postfix = varargin{end};
    
    % save out the data, Lixu, 20220318
    switch Flight_ID
        case {'FN06', 'FN19'}
            filename = ['/glade/u/home/lixujin/matlab/F0AM-4.2.1/Setups/Examples/Lixu/FIREX-AQ/output_data/pltrate', names, MECHANISM, '_', FixNOx_postfix, Flight_ID];
            save(filename, '-struct', 'SpRates');
            disp('save out required SpRates data');
        case {'RF03', 'RF07', 'RF09'}
            filename = ['/glade/u/home/lixujin/matlab/F0AM-4.2.1/Setups/Examples/Lixu/WE-CAN/output_data/pltrate', names, MECHANISM, '_', FixNOx_postfix, Flight_ID];
            save(filename, '-struct', 'SpRates');
            disp('save out required SpRates data');
        case {'P3B'}
            filename = ['/glade/u/home/lixujin/matlab/F0AM-4.2.1/Setups/Examples/Lixu/P-3B/output_data/pltrate', names, MECHANISM, '_', FixNOx_postfix, Flight_ID];
            save(filename, '-struct', 'SpRates');
            disp('save out required SpRates data');
        otherwise
    end    
