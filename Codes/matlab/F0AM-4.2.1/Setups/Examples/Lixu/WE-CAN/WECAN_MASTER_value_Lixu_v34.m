...
JHNO4_HO2_NO2_UV_VISonly = met_obs.JHNO4_HO2_NO2_UV_VISonly;
...
bkg_NO3_AMS = bkg_obs.bkg_NO3_AMS;
...
% AMS
smk_NO3_AMS = smk_obs.smk_NO3_AMS;
...
%   names               conc(ppb)               HoldMe 
    ...
    'HNO3'               smk_HNO3_UWCIMS(starting_point)+smk_NO3_AMS(starting_point)*0.4     0;...  
    %'HNO3'               smk_HNO3_UWCIMS(starting_point)                                      0;...        
    ...
    nTBUT2ENE            smk_CO_QCL(starting_point).*scale_factor.*0.33*0.001                 0;...
    nCBUT2ENE            smk_CO_QCL(starting_point).*scale_factor.*0.33*0.001                 0;...
    nPENT1ENE            max(smk_pentene_C5H10_PTR).*scale_factor                             0;...
    %nPENT1ENE            6.25E-01                                                            0;...
    nHEX1ENE             smk_CO_QCL(starting_point).*scale_factor.*0.75*0.001                 0;...
    %nHEX1ENE            1.69E+00                                                             0;...
    ...
    'C3H8'              smk_CO_QCL(starting_point).*scale_factor.*2.74.*0.001                 0;... %Lixu
    nALK4               smk_CO_QCL(starting_point).*scale_factor.*(0.71+0.27+0.3+0.15+0.28+0.23+0.23+0.25+0.071+0.052+0.034+0.015+0.017+0.033).*0.001                     0;... 
    nNC4H10             smk_CO_QCL(starting_point).*scale_factor.*0.71.*0.001                 0;...
    nIC4H10             smk_CO_QCL(starting_point).*scale_factor.*0.27.*0.001                 0;...
    nNC5H12             smk_CO_QCL(starting_point).*scale_factor.*0.3.*0.001                  0;... 
    nIC5H12             smk_CO_QCL(starting_point).*scale_factor.*0.15.*0.001                 0;...
    nNC6H14             smk_CO_QCL(starting_point).*scale_factor.*0.28.*0.001                 0;... 
    nM3PE               smk_CO_QCL(starting_point).*scale_factor.*0.23.*0.001                 0;... 
    nM22C4              smk_CO_QCL(starting_point).*scale_factor.*0.23.*0.001                 0;...
    nNC7H16             smk_CO_QCL(starting_point).*scale_factor.*0.25.*0.001                 0;...
    nNC8H18             smk_CO_QCL(starting_point).*scale_factor.*(0.071+0.052+0.034+0.015+0.017).*0.001        0;... %scalling from Permar 2020
    nNC9H20             smk_CO_QCL(starting_point).*scale_factor.*0.033.*0.001                                  0;... %scalling from Permar 2020
    ...
    nACTA              smk_Acetic_acid_C2H4O2_PTR(starting_point).*scale_factor     0;... 
    ...
    'DMS'                smk_CO_QCL(starting_point).*scale_factor*0.13/1000.0                              0;... %Lixu, check Koss 2015
    ...
    %nPHEN                smk_phenol_C6H6O_PTR(starting_point).*scale_factor                                    0;... 
    ...
    nTM123B              smk_CO_QCL(starting_point).*scale_factor*0.195.*0.001                             0;... %Peng
    %nTM123B              2.37E-01                     0;... %Peng
    nIPBENZ              smk_CO_QCL(starting_point).*scale_factor*0.03.*0.001                              0;... %Lixu
    nPBENZ               smk_CO_QCL(starting_point).*scale_factor*0.015.*0.001                             0;... %Lixu
    nMETHTOL             smk_CO_QCL(starting_point).*scale_factor*0.079.*0.001                             0;... %LIxu
    nPETHTOL             smk_CO_QCL(starting_point).*scale_factor*0.103.*0.001                             0;... %Lixu
    nTM135B              smk_CO_QCL(starting_point).*scale_factor*0.08.*0.001                              0;... %Lixu
    nOETHTOL             smk_CO_QCL(starting_point).*scale_factor*0.058.*0.001                             0;... %Lixu
...
%   names               values
    ...
    'PAN'               min(bkg_PAN)                            ;... 
    %'HNO3'              min(bkg_HNO3_UWCIMS) + min(bkg_NO3_AMS)*0.4 ;...
    'HNO3'              0 ;...
...
%to be consistent with Rickly et al. (2022) Colorado front range study.
if is_FixNOx
...
prefix = '';
ModelOptions.SavePath = ['/glade/u/home/lixujin/matlab/F0AM-4.2.1/Setups/Examples/Lixu/WE-CAN/output_data/WECAN_', MECHANISM, '_', FixNOx_postfix, Flight_ID];
...
fileoutput = [prefix, 'figures/', Flight_ID, '_O3_TS_', MECHANISM, '_', FixNOx_postfix, '.png'];
...
fileoutput = [prefix, 'figures/', Flight_ID, '_CO_TS_', MECHANISM, '_', FixNOx_postfix, '.png'];
...
fileoutput = [prefix, 'figures/', Flight_ID, '_BENZ_TS_', MECHANISM, '_', FixNOx_postfix, '.png'];
saveas(gcf, fileoutput); % Save the figure


figure
plot(obs_time, smk_NO + smk_NO2, 'b-');
% Using scatter for the observations
hold on; % This ensures the scatter plot is overlaid on the previous plot
scatter(S.Time, S.Conc.NO + S.Conc.NO2, 'r', 'filled'); % 'filled' makes the markers solid
xlabel('Reaction Time (sec)');
ylabel('NOx (ppb)');
legend('Observation', 'Model'); % Adjusted the legend to indicate observations
title('NOx Time series');
fileoutput = [prefix, 'figures/', Flight_ID, '_NOx_TS_', MECHANISM, '_', FixNOx_postfix, '.png'];
saveas(gcf, fileoutput); % Save the figure
...
% Using scatter for the observations
hold on; % This ensures the scatter plot is overlaid on the previous plot
scatter(S.Time, S.Conc.NO, 'r', 'filled'); % 'filled' makes the markers solid
xlabel('Reaction Time (sec)');
ylabel('NO (ppb)');
legend('Observation', 'Model'); % Adjusted the legend to indicate observations
title('NO Time series');
fileoutput = [prefix, 'figures/', Flight_ID, '_NO_TS_', MECHANISM, '_', FixNOx_postfix, '.png'];
saveas(gcf, fileoutput); % Save the figure

figure
plot(obs_time, smk_NO2, 'b-');
% Using scatter for the observations
hold on; % This ensures the scatter plot is overlaid on the previous plot
scatter(S.Time, S.Conc.NO2, 'r', 'filled'); % 'filled' makes the markers solid
xlabel('Reaction Time (sec)');
ylabel('NO2 (ppb)');
legend('Observation', 'Model'); % Adjusted the legend to indicate observations
title('NO2 Time series');
fileoutput = [prefix, 'figures/', Flight_ID, '_NO2_TS_', MECHANISM, '_', FixNOx_postfix, '.png'];
saveas(gcf, fileoutput); % Save the figure
...
%{
figure 
plot(obs_time, (smk_FURAN_C4H4O_PTR) +  (smk_2_Methylfuran_C5H6O_PTR) + (smk_3_Methylfuran_C5H6O_PTR) + (smk_2_5_dimethyl_furan_C6H8O), 'b-');
% Using scatter for the observations
hold on; % This ensures the scatter plot is overlaid on the previous plot
scatter(S.Time, S.Conc.(nFurans), 'r', 'filled'); % 'filled' makes the markers solid
xlabel('Reaction Time (sec)');
ylabel('Furans (ppb)');
legend('Observation', 'Model'); % Adjusted the legend to indicate observations
title('Furans Time series');
fileoutput = [prefix, 'figures/', Flight_ID, '_', nFurans, '_TS_', MECHANISM, '_', FixNOx_postfix, '.png'];
saveas(gcf, fileoutput); % Save the figure
disp('done, finish the figure plotting.');
%}
%{
figure
plot(mod_time,Sd.Conc.HNO3,'b-',...
    obs_time,(smk_HNO3_UWCIMS + smk_NO3_AMS*0.4).*(smk_CO(starting_point)-S.BkgdConc.('CO')(1))./(smk_CO-S.BkgdConc.('CO')(1)),'ro',...
    obs_time,(smk_HNO3_UWCIMS).*(smk_CO(starting_point)-S.BkgdConc.('CO')(1))./(smk_CO-S.BkgdConc.('CO')(1)),'gx', ...
    obs_time,(smk_NO3_AMS.*0.4).*(smk_CO(starting_point)-S.BkgdConc.('CO')(1))./(smk_CO-S.BkgdConc.('CO')(1)),'rd')
    
xlabel('Reaction Time (s)')
ylabel('pNO3 + HNO_3 (ppb)')
legend('Model','Obs', 'Obs_HNO3', 'Obs_pNO3')
title('pNO3 + HNO3 dilution corrected concentration') 
fileoutput = [prefix, 'figures/', Flight_ID, '_pNO3_HNO3_NEMR_', MECHANISM, '.png'];
saveas(gcf, fileoutput) % do not forget to save out the figure otherwise it cannot be displayed
%}
...
figure
plot(mod_time,Sd.Conc.HNO3,'b-',...
    obs_time,(smk_HNO3_UWCIMS - S.BkgdConc.('HNO3')(1)).*(smk_CO(starting_point)-S.BkgdConc.('CO')(1))./(smk_CO-S.BkgdConc.('CO')(1)),'ro')    
xlabel('Reaction Time (s)')
ylabel('HNO_3 (ppb)')
legend('Model','Obs')
title('HNO3 dilution corrected concentration') 
fileoutput = [prefix, 'figures/', Flight_ID, '_HNO3_NEMR_', MECHANISM, '.png'];
saveas(gcf, fileoutput) % do not forget to save out the figure otherwise it cannot be displayed
...
legend('Model','Obs')
title('O3 dilution corrected concentration ') 
fileoutput = [prefix, 'figures/', Flight_ID, '_O3_NEMR_', MECHANISM, '.png'];
saveas(gcf, fileoutput) % do not forget to save out the figure otherwise it cannot be displayed

figure
plot(mod_time,Sd.Conc.PAN,'b-',...
    obs_time,(smk_PAN-S.BkgdConc.('PAN')(1)).*(smk_CO(starting_point)-S.BkgdConc.('CO')(1))./(smk_CO-S.BkgdConc.('CO')(1)),'ro')
xlabel('Reaction Time (s)')
ylabel('PAN (ppb)')
legend('Model','Obs')
title('PAN dilution corrected concentration') 
fileoutput = [prefix, 'figures/', Flight_ID, '_PAN_NEMR_', MECHANISM, '.png'];
saveas(gcf, fileoutput) % do not forget to save out the figure otherwise it cannot be displayed
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
