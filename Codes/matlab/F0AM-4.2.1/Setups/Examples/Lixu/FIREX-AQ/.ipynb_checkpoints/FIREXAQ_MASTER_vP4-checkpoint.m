...
% Also interpolate NO and NO2 to make the fixNOx work, 20230323, LJ
NO_Interp  = interp1(ChemInput.TimeDownwind_s, ChemInput.NO,TimeInterp);
NO2_Interp = interp1(ChemInput.TimeDownwind_s, ChemInput.NO2,TimeInterp);
...
%disp(MetInput_Interp.Temperature_C);
...
Met = {...
    ...
    nJCH3CHO        MetInput_Interp.jAcetaldehyde_CH3.*ScaleFacMet;... % acetaldehyde photolysis frequency
...
%   names       conc(ppb)                                                   HoldMe
    ...
    %NOy 
    'NO'         NO_Interp.*ScaleFacNOx                                                 0;... %NO, may need adjustment for FN06
    'NO2'        NO2_Interp.*ScaleFacNOx                                                0;... %NO2, may need adjustment for FN06
    ...
    % CxHy
    'CH4'        ChemInput.CO(starting_point).*ScaleFacVOC.*102.*0.001                  0;... %scalling from Permar but Andreae has a higher value
    ...
    nPRPE        (ChemInput.Propene(starting_point) + ...
                    ChemInput.butenes(starting_point) + ...
                    ChemInput.CO(starting_point).*0.66.*0.001+ ...
                    ChemInput.Pentene_methylbutene(starting_point) + ...
                    ChemInput.CO(starting_point).*0.75.*0.001).*ScaleFacVOC             0;...
    ...
    nCBUT2ENE    ChemInput.CO(starting_point).*ScaleFacVOC.*0.33.*0.001                 0;... 
    nPENT1ENE    ChemInput.Pentene_methylbutene(starting_point).*ScaleFacVOC            0;... 
    nHEX1ENE     ChemInput.CO(starting_point).*ScaleFacVOC.*0.75.*0.001                 0;... 
    
    %{
    nPRPE        ChemInput.Propene(starting_point).*ScaleFacVOC + ...
                    ChemInput.butenes(starting_point).*ScaleFacVOC + ...
                    ChemInput.Pentene_methylbutene(starting_point).*ScaleFacVOC         0;...
    nC3H6        ChemInput.Propene(starting_point).*ScaleFacVOC + ...
                    ChemInput.butenes(starting_point).*ScaleFacVOC + ...
                    ChemInput.Pentene_methylbutene(starting_point).*ScaleFacVOC         0;...
    nBUT1ENE     0                                                                      0;... %Butenes (from iWAS)
    nTBUT2ENE    0                                                                      0;... %scalling from Temperate forest in Andreae 2019
    nCBUT2ENE    0                                                                      0;... 
    nPENT1ENE    0                                                                      0;... 
    nHEX1ENE     0                                                                      0;... 
    %}
    'C3H8'       ChemInput.CO(starting_point).*ScaleFacVOC*2.45.*0.001                  0;... 
    nALK4        ChemInput.CO(starting_point).*ScaleFacVOC.*(0.71+0.27+0.3+0.15+...
                    0.28+0.23+0.23+0.25+0.071+0.052+0.034+0.015+0.017+0.033).*0.001     0;... 
    ...
    nNC8H18      ChemInput.CO(starting_point).*ScaleFacVOC.*(0.071+0.052+...
                                                            0.034+0.015+0.017).*0.001   0;... %scalling from Permar 2020
    nNC9H20      ChemInput.CO(starting_point).*ScaleFacVOC.*0.033.*0.001                0;... %scalling from Permar 2020
    
    %{
    nALK4        ChemInput.CO(starting_point).*ScaleFacVOC.*(0.71+0.27+0.3+0.15+0.28+...
                    0.23+0.23+0.25+0.071+0.052+0.034+0.015+0.017+0.033).*0.001          0;... 
    nNC4H10      ChemInput.CO(starting_point).*ScaleFacVOC.*(0.71+0.27+0.3+0.15+0.28+...
                    0.23+0.23+0.25+0.071+0.052+0.034+0.015+0.017+0.033).*0.001          0;... 
    nIC4H10      0                                                                      0;...
    nNC5H12      0                                                                      0;... 
    nIC5H12      0                                                                      0;...
    ...
    nRCHO        ChemInput.CO(starting_point).*ScaleFacVOC.*(0.813+0.21.*0.5+...
                    0.21.*0.5+0.20+0.34+0.017).*0.001    0;... % Permar et al
    ...
    nGLYX         ChemInput.CO(starting_point).*ScaleFacVOC.*(4.82).*0.001              0;... %Glyoxal (from ACES)
    
    % Aromatics
    nBENZ        ChemInput.Benzene(starting_point).*ScaleFacVOC                         0;... %Benzene
    %nTOLU        ChemInput.CO(starting_point).*ScaleFacVOC.*1.23.*0.001                0;... %Permar et al. (2022)
    nTOLU        ChemInput.Toluene(starting_point)                                      0;... %Permar et al. (2022)
    nXYLE        ChemInput.CO(starting_point).*ScaleFacVOC.*0.53.*0.001                 0;... %
    ...
    netBENZ      ChemInput.CO(starting_point).*ScaleFacVOC.*0.53.*0.355.*0.001          0;... % Ethylbenzene
    ...
    %nAPINENE     0                                                                     0;... %Alpha Pinene
    %nBPINENE     0                                                                     0;... %Beta Pinene
    ...
    % Furans    
    nFurans      (ChemInput.Furan(starting_point) + ...
                  ChemInput.MethylFuran(starting_point) + ...
                  ChemInput.Dimethylfuran(starting_point)).*ScaleFacVOC                 0;... % Lixu
    nFURAN       ChemInput.Furan(starting_point).*ScaleFacVOC                           0;... %Furan
    nM2FURAN     ChemInput.MethylFuran(starting_point).*ScaleFacVOC*0.84                0;... %Methylfuran (~50% of signal)
    nM3F         ChemInput.MethylFuran(starting_point).*ScaleFacVOC*0.16                0;... %Methylfuran (~50% of signal)
    nDIMEFURAN   ChemInput.Dimethylfuran(starting_point).*ScaleFacVOC                   0;... %Dimethyl furan (~50% of signal, according to Abby)
    
    nFurfurals   (ChemInput.Furfural(starting_point)+...
                    ChemInput.MethylFurfural(starting_point)).*ScaleFacVOC              0;...
    ...
    nCATECHOL    ChemInput.BenzeneDiol(starting_point).*ScaleFacVOC                     0;... %Catechol, Lixu, do not found in the input
    ...
    % For FIREX-AQ
    nACO2H       ChemInput.Acrylicacid(starting_point).*ScaleFacVOC + ...
                    ChemInput.CO(starting_point).*ScaleFacVOC*0.36/1000.0               0;... %Acrylic Acid (assume same as methyl glyoxal), not sure
    ...
    %nM3HEXANO3         ChemInput.CO(starting_point).*ScaleFacVOC*(0.64+0.21)/1000.0    0;... % this will mess up with ANs comparison, mute it for now
    ...
    %nC108NO3           ChemInput.CO(starting_point).*ScaleFacVOC*0.06/1000.0             0;...% this will mess up with ANs comparison, mute it for now
...
%   names               values
    ...
    nFurans      ChemInput.BGMixingRatios(13)+...
                 ChemInput.BGMixingRatios(15)+...
                 ChemInput.BGMixingRatios(9)       ;
    ...
    nFurfurals   ChemInput.BGMixingRatios(14)+...
                 ChemInput.BGMixingRatios(4)       ;
...
% ----
%{
PlotRates(nAPINENE,S,1000,'sumEq',1,'scale',1)
fileoutput = [prefix, 'figures/', Flight_ID, '_AP_rates_', MECHANISM, '.png'];
saveas(gcf, fileoutput) % do not forget to save out the figure otherwise it cannot be displayed
PlotRates(nBPINENE,S,1000,'sumEq',1,'scale',1)
fileoutput = [prefix, 'figures/', Flight_ID, '_BP_rates_', MECHANISM, '.png'];
saveas(gcf, fileoutput) % do not forget to save out the figure otherwise it cannot be displayed
%}
