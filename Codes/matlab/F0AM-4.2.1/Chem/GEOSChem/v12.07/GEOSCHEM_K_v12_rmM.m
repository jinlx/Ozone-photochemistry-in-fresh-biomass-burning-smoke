...
K2 = 2.10E-33.*exp(920./T);
krx(:,i) = (K1 + K2).*(1.+1.4E-21.*exp(2200./T));
...
KCO1=KLO1.*0.6.^FEXP1./(1.0+XYRAT1);
...
i=i+1;
Knames{i}   = 'K_N2O5_M';
LPL         = 7.4E-4*(300./T).^(-4.4).* exp(-11000./T);
HPL         = 5.18E14*(300./T).^(-0.7).*exp(-11000./T);
krx(:,i)    = Troe_GC(LPL,HPL);

i=i+1;
Knames{i}   = 'K_HO2_NO2';
LPL         = 2.0E-31.*(T./300).^(-3.4) ;
...
i=i+1;
Knames{i}   = 'K_HNO4_M';
LPL         = 9.52E-5*(300./T).^(-3.4).* exp(-10900./T);
HPL         = 1.38E15*(300./T).^(-1.1).*exp(-10900./T);
krx(:,i)    = Troe_GC(LPL,HPL);

i=i+1;
Knames{i}   = 'K_MCO3_NO2';
LPL         = (9.70E-29.*(T./300).^-5.6);
